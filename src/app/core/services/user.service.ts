...
import { CreateUserProfileDTO, UpdateUserProfileDTO, UserProfileDTO } from '../models/user.model';
...
export class UserService {
  ...
  getUser(id: number | string): Observable<UserProfileDTO> {
    return this.http.get<any>(`${this.API_URL}/${id}`).pipe(
      map((resp) => this.mapUserProfile(resp?.data ?? resp))
    );
  }

  createUser(data: CreateUserProfileDTO): Observable<UserProfileDTO> {
    return this.http.post<any>(this.API_URL, data).pipe(
      map(raw => this.mapUserProfile(raw?.data ?? raw))
    );
  }

  updateUser(id: number | string, data: UpdateUserProfileDTO): Observable<UserProfileDTO> {
  ...
  updateProfilePicture(id: number | string, file: File): Observable<UserProfileDTO> {
    const formData = new FormData();
    formData.append('file', file);
    return this.http.put<any>(`${this.API_URL}/${id}/profile-picture`, formData).pipe(
      map(raw => this.mapUserProfile(raw?.data ?? raw))
    );
  }

  activateUserProfile(id: number | string): Observable<UserProfileDTO> {
    return this.http.patch<any>(`${this.API_URL}/${id}/activate`, {}).pipe(
      map(raw => this.mapUserProfile(raw?.data ?? raw))
    );
  }

  deactivateUserProfile(id: number | string): Observable<UserProfileDTO> {
    return this.http.patch<any>(`${this.API_URL}/${id}/deactivate`, {}).pipe(
      map(raw => this.mapUserProfile(raw?.data ?? raw))
    );
  }

  private mapUserProfile(raw: any): UserProfileDTO {
    const id = raw?.id ?? raw?.userProfileId ?? raw?.idUserProfile ?? raw?.id_user_profile ?? raw?.profileId ?? raw?.idProfile ?? raw?.userId ?? raw?.uuid;
    return {
